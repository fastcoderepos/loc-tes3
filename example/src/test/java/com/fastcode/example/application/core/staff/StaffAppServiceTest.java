...
import com.fastcode.example.domain.core.staff.*;
...
//		search.setOperator("equals");
   ...
   //Store
	...
	public void GetStore_IfStaffIdAndStoreIdIsNotNullAndStaffExists_ReturnStore() {
		...
		Mockito.when(_staffRepository.findById(any(Integer.class))).thenReturn(staffOptional);
		Mockito.when(staff.getStore()).thenReturn(storeEntity);
		Assertions.assertThat(_appService.getStore(ID)).isEqualTo(_mapper.storeEntityToGetStoreOutput(storeEntity, staff));
	}
