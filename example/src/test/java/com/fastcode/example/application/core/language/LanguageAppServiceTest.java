...
import com.fastcode.example.domain.core.language.*;
...
public class LanguageAppServiceTest {
	...
	public void findLanguageById_IdIsNotNullAndIdExists_ReturnLanguage() {
	    ...
	    Assertions.assertThat(_appService.findById(ID )).isEqualTo(_mapper.languageEntityToFindLanguageByIdOutput(language));
	}
    ...
    public void createLanguage_LanguageIsNotNullAndLanguageDoesNotExist_StoreLanguage() { 
        ...
        Mockito.when(_mapper.createLanguageInputToLanguageEntity(any(CreateLanguageInput.class))).thenReturn(languageEntity); 
        Mockito.when(_languageRepository.save(any(LanguageEntity.class))).thenReturn(languageEntity);

	   	Assertions.assertThat(_appService.create(languageInput)).isEqualTo(_mapper.languageEntityToCreateLanguageOutput(languageEntity));
	...
	public void updateLanguage_LanguageIdIsNotNullAndIdExists_ReturnUpdatedLanguage() {
		...
		Mockito.when(_mapper.updateLanguageInputToLanguageEntity(any(UpdateLanguageInput.class))).thenReturn(languageEntity);
		Mockito.when(_languageRepository.save(any(LanguageEntity.class))).thenReturn(languageEntity);
		Assertions.assertThat(_appService.update(ID,language)).isEqualTo(_mapper.languageEntityToUpdateLanguageOutput(languageEntity));
	}
...
//		search.setOperator("equals");
		...
		Mockito.when(_languageRepository.findAll(any(Predicate.class),any(Pageable.class))).thenReturn(foundPage);
...
//		search.setOperator("equals");
	...
	public void  search_SearchIsNotNullAndSearchContainsCaseThree_ReturnBooleanBuilder() throws Exception {
		...
		Mockito.doReturn(builder).when(_appService).searchKeyValuePair(any(QLanguageEntity.class), any(HashMap.class), any(HashMap.class));
