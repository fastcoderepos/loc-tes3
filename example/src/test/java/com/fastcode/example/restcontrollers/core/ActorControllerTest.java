...
public class ActorControllerTest {
	...
	public void setup() {
		...
		final ActorController actorController = new ActorController(actorAppService, filmActorAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(actorController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/actor/" + actor.getActorId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/actor/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateActor_ActorDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/actor").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteActor_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/actor/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a actor with a id=999"));
    ...
    //    Mockito.when(actorAppService.findById(entity.getActorId())).thenReturn(output);
		...
		mvc.perform(delete("/actor/" + entity.getActorId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateActor_ActorDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(actor);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/actor/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Actor with id=999 not found."));
	}    
	...
	public void UpdateActor_ActorExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/actor/" + entity.getActorId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/actor?search=actorId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/actor?search=actoractorId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property actoractorId not found!"));
	...
	public void GetFilmActors_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/actor/1/filmActors?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetFilmActors_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(actorAppService.parseFilmActorsJoinColumn("actorId")).thenReturn(joinCol);
		mvc.perform(get("/actor/1/filmActors?search=actorId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetFilmActors_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/actor/1/filmActors?search=actorId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
