...
public class AddressControllerTest {
	...
	public void setup() {
		...
		final AddressController addressController = new AddressController(addressAppService, cityAppService, customerAppService, customUserAppService, staffAppService, storeAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(addressController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/address/" + address.getAddressId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/address/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateAddress_AddressDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/address").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteAddress_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/address/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a address with a id=999"));
    ...
    //    Mockito.when(addressAppService.findById(entity.getAddressId())).thenReturn(output);
		...
		mvc.perform(delete("/address/" + entity.getAddressId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateAddress_AddressDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(address);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/address/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Address with id=999 not found."));
	}    
	...
	public void UpdateAddress_AddressExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/address/" + entity.getAddressId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/address?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address?search=addressaddressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property addressaddressId not found!"));
	...
	public void GetCity_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address/999/city")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetCity_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/address/" + address.getAddressId()+ "/city")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetCustomers_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address/1/customers?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetCustomers_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(addressAppService.parseCustomersJoinColumn("addressId")).thenReturn(joinCol);
		mvc.perform(get("/address/1/customers?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetCustomers_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/address/1/customers?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
	
    @Test
	public void GetCustomUsers_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
	
		Map<String,String> joinCol = new HashMap<String,String>();
		joinCol.put("addressId", "1");

		Mockito.when(addressAppService.parseCustomUsersJoinColumn("addressid")).thenReturn(joinCol);
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address/1/customUsers?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	@Test
	public void GetCustomUsers_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {

		Map<String,String> joinCol = new HashMap<String,String>();
		joinCol.put("addressId", "1");
		
        Mockito.when(addressAppService.parseCustomUsersJoinColumn("addressId")).thenReturn(joinCol);
		mvc.perform(get("/address/1/customUsers?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	
	@Test
	public void GetCustomUsers_searchIsNotEmpty() {
	
		Mockito.when(addressAppService.parseCustomUsersJoinColumn(anyString())).thenReturn(null);
	 		  		    		  
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/address/1/customUsers?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
	...
	public void GetStaffs_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address/1/staffs?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetStaffs_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(addressAppService.parseStaffsJoinColumn("addressId")).thenReturn(joinCol);
		mvc.perform(get("/address/1/staffs?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetStaffs_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/address/1/staffs?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
	...
	public void GetStores_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/address/1/stores?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetStores_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(addressAppService.parseStoresJoinColumn("addressId")).thenReturn(joinCol);
		mvc.perform(get("/address/1/stores?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetStores_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/address/1/stores?search=addressId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
