...
import com.fastcode.example.domain.core.filmcategory.IFilmCategoryRepository;
...
public class FilmCategoryControllerTest {
	...
	public static void cleanup() {
		...
		em.createNativeQuery("truncate table public.film_category").executeUpdate();
	...
	public FilmCategoryEntity createEntity() {
		...
		filmCategoryEntity.setCategoryId(Short.parseShort(category.getCategoryId().toString()));
		filmCategoryEntity.setFilm(film);
		filmCategoryEntity.setFilmId(Short.parseShort(film.getFilmId().toString()));
	...
	public void setup() {
		...
		final FilmCategoryController filmCategoryController = new FilmCategoryController(filmCategoryAppService, categoryAppService, filmAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(filmCategoryController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/filmCategory/categoryId=" + filmCategory.getCategoryId()+ ",filmId=" + filmCategory.getFilmId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/filmCategory/categoryId=999,filmId=999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateFilmCategory_FilmCategoryDoesNotExist_ReturnStatusOk() throws Exception {
		...
		filmCategoryInput.setCategoryId(Short.parseShort(category.getCategoryId().toString()));
	    
		FilmEntity film =  createFilmEntity();

		filmCategoryInput.setFilmId(Short.parseShort(film.getFilmId().toString()));
		...
		mvc.perform(post("/filmCategory").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteFilmCategory_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/filmCategory/categoryId=999,filmId=999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a filmCategory with a id=categoryId=999,filmId=999"));
	...
	public void Delete_IdIsValid_ReturnStatusNoContent() throws Exception {
		...
		CategoryEntity category = createCategoryEntity();
		entity.setCategoryId(Short.parseShort(category.getCategoryId().toString()));
	...
	//    Mockito.when(filmCategoryAppService.findById(new FilmCategoryId(entity.getCategoryId(), entity.getFilmId()))).thenReturn(output);
        Mockito.doReturn(output).when(filmCategoryAppService).findById(new FilmCategoryId(entity.getCategoryId(), entity.getFilmId()));
        
		mvc.perform(delete("/filmCategory/categoryId="+ entity.getCategoryId()+ ",filmId="+ entity.getFilmId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateFilmCategory_FilmCategoryDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(filmCategory);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/filmCategory/categoryId=999,filmId=999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. FilmCategory with id=categoryId=999,filmId=999 not found."));
	}    
	...
	public void UpdateFilmCategory_FilmCategoryExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/filmCategory/categoryId=" + entity.getCategoryId()+ ",filmId=" + entity.getFilmId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/filmCategory?search=categoryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/filmCategory?search=filmCategorycategoryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property filmCategorycategoryId not found!"));
	...
	public void GetCategory_IdIsNotEmptyAndIdIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/filmCategory/categoryId999/category")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetCategory_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/filmCategory/categoryId=999,filmId=999/category")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetCategory_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/filmCategory/categoryId=" + filmCategory.getCategoryId()+ ",filmId=" + filmCategory.getFilmId()+ "/category")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetFilm_IdIsNotEmptyAndIdIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/filmCategory/categoryId999/film")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid id=categoryId999"));
	...
	public void GetFilm_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/filmCategory/categoryId=999,filmId=999/film")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetFilm_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/filmCategory/categoryId=" + filmCategory.getCategoryId()+ ",filmId=" + filmCategory.getFilmId()+ "/film")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
