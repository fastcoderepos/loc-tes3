...
import com.fastcode.example.application.core.store.dto.*;
...
public class StoreControllerTest {
	...
	public static void cleanup() {
		...
		em.createNativeQuery("truncate table public.store").executeUpdate();
	...
	public StoreEntity createEntity() {
    	...
    	storeEntity.setLastUpdate(SearchUtils.stringToLocalDateTime("1996-09-01 09:15:22"));
	...
	public CreateStoreInput createStoreInput() {
    	...
    	storeInput.setLastUpdate(SearchUtils.stringToLocalDateTime("1996-08-10 05:25:22"));
	...
	public StoreEntity createNewEntity() {
    	...
    	store.setLastUpdate(SearchUtils.stringToLocalDateTime("1996-08-11 05:35:22"));
		store.setStoreId(3);
	...
	public StoreEntity createUpdateEntity() {
		StoreEntity store = new StoreEntity();
    	store.setLastUpdate(SearchUtils.stringToLocalDateTime("1996-09-09 05:45:22"));
		store.setStoreId(4);
	...
	public void setup() {
		...
		final StoreController storeController = new StoreController(storeAppService, addressAppService, staffAppService,
	logHelper,env);
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/store/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateStore_StoreDoesNotExist_ReturnStatusOk() throws Exception {
		...
		storeInput.setAddressId(Short.parseShort(address.getAddressId().toString()));
		countStaff++;
	    
		StaffEntity staff =  createStaffEntity();

		storeInput.setManagerStaffId(Short.parseShort(staff.getStaffId().toString()));
		...
		mvc.perform(post("/store").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteStore_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/store/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a store with a id=999"));
    ...
    //    Mockito.when(storeAppService.findById(entity.getStoreId())).thenReturn(output);
		...
		mvc.perform(delete("/store/" + entity.getStoreId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateStore_StoreDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		store.setLastUpdate(SearchUtils.stringToLocalDateTime("1996-09-28 07:15:22"));
		store.setStoreId(999);

		ObjectWriter ow = new ObjectMapper().registerModule(new JavaTimeModule()).disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS).writer().withDefaultPrettyPrinter();
		String json = ow.writeValueAsString(store);

		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/store/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Store with id=999 not found."));
	}    
	...
	public void UpdateStore_StoreExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/store/" + entity.getStoreId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/store?search=storeId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/store?search=storestoreId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property storestoreId not found!"));
	...
	public void GetAddress_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/store/999/address")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetAddress_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/store/" + store.getStoreId()+ "/address")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetStaff_IdIsNotEmptyAndIdDoesNotExist_ReturnNotFound() {
	   ...
	   org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/store/999/staff")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Not found"));
	...
	public void GetStaff_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
	   ...
	   mvc.perform(get("/store/" + store.getStoreId()+ "/staff")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
