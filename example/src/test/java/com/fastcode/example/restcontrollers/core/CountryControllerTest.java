...
public class CountryControllerTest {
	...
	public void setup() {
		...
		final CountryController countryController = new CountryController(countryAppService, cityAppService,
	logHelper,env);
		when(logHelper.getLogger()).thenReturn(loggerMock);
		doNothing().when(loggerMock).error(anyString());

		this.mvc = MockMvcBuilders.standaloneSetup(countryController)
	...
	public void FindById_IdIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/country/" + country.getCountryId()+"/")
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/country/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void CreateCountry_CountryDoesNotExist_ReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/country").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void DeleteCountry_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/country/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a country with a id=999"));
    ...
    //    Mockito.when(countryAppService.findById(entity.getCountryId())).thenReturn(output);
		...
		mvc.perform(delete("/country/" + entity.getCountryId()+"/")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void UpdateCountry_CountryDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(country);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(put("/country/999").contentType(MediaType.APPLICATION_JSON).content(json))
					.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. Country with id=999 not found."));
	}    
	...
	public void UpdateCountry_CountryExists_ReturnStatusOk() throws Exception {
		...
		mvc.perform(put("/country/" + entity.getCountryId()+"/").contentType(MediaType.APPLICATION_JSON).content(json))
		.andExpect(status().isOk());
	...
	public void FindAll_SearchIsNotNullAndPropertyIsValid_ReturnStatusOk() throws Exception {
		...
		mvc.perform(get("/country?search=countryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk());
	}    
	...
	public void FindAll_SearchIsNotNullAndPropertyIsNotValid_ThrowException() throws Exception {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/country?search=countrycountryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property countrycountryId not found!"));
	...
	public void GetCitys_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/country/1/citys?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetCitys_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(countryAppService.parseCitysJoinColumn("countryId")).thenReturn(joinCol);
		mvc.perform(get("/country/1/citys?search=countryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetCitys_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/country/1/citys?search=countryId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
