...
public class RentalController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a rental with a id=%s", id)));
	...
	public ResponseEntity<UpdateRentalOutput> update(@PathVariable String id, @RequestBody @Valid UpdateRentalInput rental) {

	    FindRentalByIdOutput currentRental = _rentalAppService.findById(Integer.valueOf(id));
		Optional.ofNullable(currentRental).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Rental with id=%s not found.", id)));
	...
	@RequestMapping(value = "/{id}/inventory", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
