...
import com.fastcode.example.application.core.film.dto.FindFilmByIdOutput;
import java.util.*;
import java.time.*;
...
public class LanguageController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a language with a id=%s", id)));
	...
	public ResponseEntity<UpdateLanguageOutput> update(@PathVariable String id, @RequestBody @Valid UpdateLanguageInput language) {

	    FindLanguageByIdOutput currentLanguage = _languageAppService.findById(Integer.valueOf(id));
		Optional.ofNullable(currentLanguage).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Language with id=%s not found.", id)));
	...
	@RequestMapping(value = "/{id}/films", method = RequestMethod.GET, consumes = {"application/json"}, produces = {"application/json"})
	public ResponseEntity getFilms(@PathVariable String id, @RequestParam(value="search", required=false) String search, @RequestParam(value = "offset", required=false) String offset, @RequestParam(value = "limit", required=false) String limit, Sort sort)throws Exception {
   		if (offset == null) { offset = env.getProperty("fastCode.offset.default"); }
		if (limit == null) { limit = env.getProperty("fastCode.limit.default"); }
