...
public class StoreController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a store with a id=%s", id)));
	...
	public ResponseEntity<UpdateStoreOutput> update(@PathVariable String id, @RequestBody @Valid UpdateStoreInput store) {

	    FindStoreByIdOutput currentStore = _storeAppService.findById(Integer.valueOf(id));
		Optional.ofNullable(currentStore).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Store with id=%s not found.", id)));
