...
import com.fastcode.example.application.core.actor.IActorAppService;
...
public class ActorController {
	...
	public void delete(@PathVariable String id) {
    	...
    	Optional.ofNullable(output).orElseThrow(() -> new EntityNotFoundException(String.format("There does not exist a actor with a id=%s", id)));
	...
	public ResponseEntity<UpdateActorOutput> update(@PathVariable String id, @RequestBody @Valid UpdateActorInput actor) {

	    FindActorByIdOutput currentActor = _actorAppService.findById(Integer.valueOf(id));
		Optional.ofNullable(currentActor).orElseThrow(() -> new EntityNotFoundException(String.format("Unable to update. Actor with id=%s not found.", id)));
