...
import com.fastcode.example.domain.core.actor.QActorEntity;
...
public class ActorAppService implements IActorAppService {
	...
	@NonNull protected final IActorMapper mapper;
	...
	public CreateActorOutput create(CreateActorInput input) {
		...
		ActorEntity createdActor = _actorRepository.save(actor);
	...
	public UpdateActorOutput update(Integer actorId, UpdateActorInput input) {
		...
		ActorEntity updatedActor = _actorRepository.save(actor);
	...
	public void delete(Integer actorId) {
		...
		ActorEntity existing = _actorRepository.findById(actorId).orElse(null); 
	 	_actorRepository.delete(existing);
	}
	
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public FindActorByIdOutput findById(Integer actorId) {

		ActorEntity foundActor = _actorRepository.findById(actorId).orElse(null);
		if (foundActor == null)  
			return null; 
	...
	public List<FindActorByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<ActorEntity> foundActor = _actorRepository.findAll(search(search), pageable);
	...
	protected BooleanBuilder searchKeyValuePair(QActorEntity actor, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			if(details.getKey().replace("%20","").trim().equals("actorId")) {
				if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(actor.actorId.eq(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(actor.actorId.ne(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(actor.actorId.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getStartingValue()))
                	   builder.and(actor.actorId.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(actor.actorId.loe(Integer.valueOf(details.getValue().getEndingValue())));
				}
			}
            if(details.getKey().replace("%20","").trim().equals("firstName")) {
				if(details.getValue().getOperator().equals("contains"))
					builder.and(actor.firstName.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				else if(details.getValue().getOperator().equals("equals"))
					builder.and(actor.firstName.eq(details.getValue().getSearchValue()));
				else if(details.getValue().getOperator().equals("notEqual"))
					builder.and(actor.firstName.ne(details.getValue().getSearchValue()));
			}
            if(details.getKey().replace("%20","").trim().equals("lastName")) {
				if(details.getValue().getOperator().equals("contains"))
					builder.and(actor.lastName.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				else if(details.getValue().getOperator().equals("equals"))
					builder.and(actor.lastName.eq(details.getValue().getSearchValue()));
				else if(details.getValue().getOperator().equals("notEqual"))
					builder.and(actor.lastName.ne(details.getValue().getSearchValue()));
			}
			if(details.getKey().replace("%20","").trim().equals("lastUpdate")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue()) !=null)
					builder.and(actor.lastUpdate.eq(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
					...
					builder.and(actor.lastUpdate.ne(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   ...
				   if(startLocalDateTime!=null && endLocalDateTime!=null)	 
					   builder.and(actor.lastUpdate.between(startLocalDateTime,endLocalDateTime));
				   else if(endLocalDateTime!=null)
					   builder.and(actor.lastUpdate.loe(endLocalDateTime));
