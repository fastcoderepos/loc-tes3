...
import com.fastcode.example.domain.core.language.QLanguageEntity;
...
public class LanguageAppService implements ILanguageAppService {
	...
	@NonNull protected final ILanguageMapper mapper;
	...
	public CreateLanguageOutput create(CreateLanguageInput input) {
		...
		LanguageEntity createdLanguage = _languageRepository.save(language);
	...
	public UpdateLanguageOutput update(Integer languageId, UpdateLanguageInput input) {
		...
		LanguageEntity updatedLanguage = _languageRepository.save(language);
	...
	public void delete(Integer languageId) {
		...
		LanguageEntity existing = _languageRepository.findById(languageId).orElse(null); 
	 	_languageRepository.delete(existing);
	}
	
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public FindLanguageByIdOutput findById(Integer languageId) {

		LanguageEntity foundLanguage = _languageRepository.findById(languageId).orElse(null);
	...
	public List<FindLanguageByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<LanguageEntity> foundLanguage = _languageRepository.findAll(search(search), pageable);
	...
	protected BooleanBuilder searchKeyValuePair(QLanguageEntity language, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			if(details.getKey().replace("%20","").trim().equals("languageId")) {
				if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(language.languageId.eq(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(language.languageId.ne(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(language.languageId.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getStartingValue()))
                	   builder.and(language.languageId.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(language.languageId.loe(Integer.valueOf(details.getValue().getEndingValue())));
				}
			}
			if(details.getKey().replace("%20","").trim().equals("lastUpdate")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue()) !=null)
					builder.and(language.lastUpdate.eq(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
					...
					builder.and(language.lastUpdate.ne(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   ...
				   if(startLocalDateTime!=null && endLocalDateTime!=null)	 
					   builder.and(language.lastUpdate.between(startLocalDateTime,endLocalDateTime));
				   else if(endLocalDateTime!=null)
					   builder.and(language.lastUpdate.loe(endLocalDateTime));
                   else if(startLocalDateTime!=null)
                	   builder.and(language.lastUpdate.goe(startLocalDateTime));  
                 }
                   
			}
            if(details.getKey().replace("%20","").trim().equals("name")) {
				if(details.getValue().getOperator().equals("contains"))
					builder.and(language.name.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				else if(details.getValue().getOperator().equals("equals"))
					builder.and(language.name.eq(details.getValue().getSearchValue()));
				else if(details.getValue().getOperator().equals("notEqual"))
					builder.and(language.name.ne(details.getValue().getSearchValue()));
			}
