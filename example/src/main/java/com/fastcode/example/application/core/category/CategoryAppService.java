...
import com.fastcode.example.domain.core.category.QCategoryEntity;
...
public class CategoryAppService implements ICategoryAppService {
	...
	@NonNull protected final ICategoryMapper mapper;
	...
	public CreateCategoryOutput create(CreateCategoryInput input) {
		...
		CategoryEntity createdCategory = _categoryRepository.save(category);
	...
	public UpdateCategoryOutput update(Integer categoryId, UpdateCategoryInput input) {
		...
		CategoryEntity updatedCategory = _categoryRepository.save(category);
	...
	public void delete(Integer categoryId) {
		...
		CategoryEntity existing = _categoryRepository.findById(categoryId).orElse(null); 
	...
	public FindCategoryByIdOutput findById(Integer categoryId) {
		...
		CategoryEntity foundCategory = _categoryRepository.findById(categoryId).orElse(null);
	...
	public List<FindCategoryByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {
		...
		Page<CategoryEntity> foundCategory = _categoryRepository.findAll(search(search), pageable);
	...
	protected BooleanBuilder searchKeyValuePair(QCategoryEntity category, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			if(details.getKey().replace("%20","").trim().equals("categoryId")) {
				if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(category.categoryId.eq(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(category.categoryId.ne(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(category.categoryId.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getStartingValue()))
                	   builder.and(category.categoryId.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(category.categoryId.loe(Integer.valueOf(details.getValue().getEndingValue())));
				}
			}
			if(details.getKey().replace("%20","").trim().equals("lastUpdate")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue()) !=null)
					builder.and(category.lastUpdate.eq(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
					...
					builder.and(category.lastUpdate.ne(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   ...
				   if(startLocalDateTime!=null && endLocalDateTime!=null)	 
					   builder.and(category.lastUpdate.between(startLocalDateTime,endLocalDateTime));
				   else if(endLocalDateTime!=null)
					   builder.and(category.lastUpdate.loe(endLocalDateTime));
                   else if(startLocalDateTime!=null)
                	   builder.and(category.lastUpdate.goe(startLocalDateTime));  
                 }
                   
			}
            if(details.getKey().replace("%20","").trim().equals("name")) {
				if(details.getValue().getOperator().equals("contains"))
					builder.and(category.name.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				else if(details.getValue().getOperator().equals("equals"))
					builder.and(category.name.eq(details.getValue().getSearchValue()));
				else if(details.getValue().getOperator().equals("notEqual"))
					builder.and(category.name.ne(details.getValue().getSearchValue()));
			}
