package com.fastcode.example.application.core.payment;

import lombok.NonNull;
import lombok.RequiredArgsConstructor;
import com.fastcode.example.application.core.payment.dto.*;
import com.fastcode.example.domain.core.payment.IPaymentRepository;
import com.fastcode.example.domain.core.payment.QPaymentEntity;
import com.fastcode.example.domain.core.payment.PaymentEntity;
...
public class PaymentAppService implements IPaymentAppService {
	...
	@NonNull protected final IPaymentRepository _paymentRepository;
	...
	@NonNull protected final IPaymentMapper mapper;
	...
	public CreatePaymentOutput create(CreatePaymentInput input) {
		...
		StaffEntity foundStaff = null;
	  	if(input.getCustomerId()!=null) {
		    foundCustomer = _customerRepository.findById(Integer.parseInt(input.getCustomerId().toString())).orElse(null);
	...
	protected BooleanBuilder searchKeyValuePair(QPaymentEntity payment, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			if(details.getKey().replace("%20","").trim().equals("amount")) {
				if(details.getValue().getOperator().equals("equals") && NumberUtils.isCreatable(details.getValue().getSearchValue()))
					builder.and(payment.amount.eq(new BigDecimal(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && NumberUtils.isCreatable(details.getValue().getSearchValue()))
					builder.and(payment.amount.ne(new BigDecimal(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   if(NumberUtils.isCreatable(details.getValue().getStartingValue()) && NumberUtils.isCreatable(details.getValue().getEndingValue()))
                	   builder.and(payment.amount.between(new BigDecimal(details.getValue().getStartingValue()), new BigDecimal(details.getValue().getEndingValue())));
                   else if(NumberUtils.isCreatable(details.getValue().getStartingValue()))
                	   builder.and(payment.amount.goe(new BigDecimal(details.getValue().getStartingValue())));
                   else if(NumberUtils.isCreatable(details.getValue().getEndingValue()))
                	   builder.and(payment.amount.loe(new BigDecimal(details.getValue().getEndingValue())));
				}
			}		
			if(details.getKey().replace("%20","").trim().equals("paymentDate")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue()) !=null)
					builder.and(payment.paymentDate.eq(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
					...
					builder.and(payment.paymentDate.ne(SearchUtils.stringToLocalDateTime(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   ...
				   if(startLocalDateTime!=null && endLocalDateTime!=null)	 
					   builder.and(payment.paymentDate.between(startLocalDateTime,endLocalDateTime));
				   else if(endLocalDateTime!=null)
					   builder.and(payment.paymentDate.loe(endLocalDateTime));
                   else if(startLocalDateTime!=null)
                	   builder.and(payment.paymentDate.goe(startLocalDateTime));  
                 }
			...
			if(details.getKey().replace("%20","").trim().equals("paymentId")) {
				if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(payment.paymentId.eq(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && StringUtils.isNumeric(details.getValue().getSearchValue()))
					builder.and(payment.paymentId.ne(Integer.valueOf(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   if(StringUtils.isNumeric(details.getValue().getStartingValue()) && StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(payment.paymentId.between(Integer.valueOf(details.getValue().getStartingValue()), Integer.valueOf(details.getValue().getEndingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getStartingValue()))
                	   builder.and(payment.paymentId.goe(Integer.valueOf(details.getValue().getStartingValue())));
                   else if(StringUtils.isNumeric(details.getValue().getEndingValue()))
                	   builder.and(payment.paymentId.loe(Integer.valueOf(details.getValue().getEndingValue())));
				}
			}
